<div class="py-6 sm:py-12 bg-slate-50 min-h-screen"
        ...
        <div class="mb-8 p-6 bg-slate-900 rounded-[32px] text-white flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-xl relative overflow-hidden">
            <div class="relative z-10">
                ...
                <div class="flex items-center gap-3">
                    <div class="p-2 bg-white/10 rounded-xl"><svg class="w-5 h-5 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="text-right relative z-10">
                ...
                <p class="text-3xl font-black italic tracking-tighter">Rp {{ number_format($payrolls->sum(fn($p) => $p->items->sum('nominal')), 0, ',', '.') }}</p>
            </div>
            <!-- Dekorasi -->
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-blue-50/20 transition-colors group">
                        <td class="px-8 py-6">
                            <div class="flex items-center gap-4">
                                <div class="h-10 w-10 rounded-xl bg-slate-100 flex items-center justify-center font-black text-slate-400 text-xs border border-slate-200 group-hover:bg-primary group-hover:text-white transition-colors">
                                    {{ substr($pay->officer->member->churchPerson->full_name ?? '?', 0, 1) }}
                                </div>
                                <div>
                                    <div class="font-black text-slate-900 text-base leading-tight uppercase italic">{{ $pay->officer->member->churchPerson->full_name ?? 'DATA HILANG' }}</div>
                        ...
                        <td class="px-6 py-6 text-right font-bold text-emerald-600">
                            Rp {{ number_format($pay->items->where('jenis','penerimaan')->sum('nominal_snapshot'), 0, ',', '.') }}
                        </td>

                        <td class="px-6 py-6 text-right font-bold text-rose-500">
                            - Rp {{ number_format($pay->items->where('jenis','potongan')->sum('nominal_snapshot'), 0, ',', '.') }}
                        </td>

                        <td class="px-6 py-6 text-right font-black text-slate-900 text-base italic">
                            Rp {{ number_format(
        $pay->items->where('jenis','penerimaan')->sum('nominal_snapshot') -
        $pay->items->where('jenis','potongan')->sum('nominal_snapshot')
        , 0, ',', '.'
    ) }}
                        </td>
    ...
    {{ $pay->status_label == 'paid' ? 'bg-emerald-100 text-emerald-700' : ($pay->status_label == 'cicil' ? 'bg-blue-100 text-blue-700' : 'bg-rose-100 text-rose-700') }}">
                                    {{ $pay->status_label }}
                                </span>

                                @if($pay->items->sum('nominal_bayar') > 0)
                                <span class="text-[8px] font-bold text-slate-400 mt-1">Masuk: Rp {{ number_format($pay->items->sum('nominal_bayar'), 0, ',', '.') }}</span>
                                @endif
                            </div>
                        </td>
                        ...
                        <td class="px-8 py-6 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <button wire:click="openPaymentModal({{ $pay->id }})" class="p-2 bg-white border border-slate-200 text-slate-400 rounded-xl hover:bg-slate-50 hover:text-primary transition-all shadow-sm">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                </button>
                                @else
                                <a href="{{ route('payroll.slip', $pay->id) }}" target="_blank" class="px-4 py-2 bg-slate-900 text-white rounded-xl font-black text-[9px] uppercase tracking-widest hover:bg-primary transition-all flex items-center gap-2">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div x-show="showPay" x-cloak class="fixed inset-0 z-[100] overflow-y-auto">
        ...
        init() {
            this.$watch('localNominal', v => {
                if (this.$refs.payInput) this.$refs.payInput.value = this.formatRupiah(v);
            });
        },
        formatRupiah(v) {
            if(!v) return '';
            // hapus semua non-digit untuk dikirim ke Livewire
            ...
            return num.replace(/\B(?=(\d{3})+(?!\d))/g,'.'); 
                ...
                <div class="mb-10 text-center">
                    ...
                    <p class="text-xs font-bold text-slate-400 mt-2 italic">{{ \Carbon\Carbon::create()->month($selectedPayrollDetails->bulan)->isoFormat('MMMM') }} {{ $selectedPayrollDetails->tahun }}</p>
                </div>
                @endif

                <form wire:submit.prevent="savePayment" class="space-y-8 text-left">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input x-ref="payInput" type="text"
                                x-on:input="localNominal = $el.value.replace(/\D/g,''); $el.value = formatRupiah($el.value)"
